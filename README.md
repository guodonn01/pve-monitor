...
List Status: It prints a report of all nodes and the current status of their KVMs (QEMU) and Containers (LXC).

Rule-Based Monitoring: It fetches performance data for each VM and checks it against a set of rules defined in rules.json. If a rule is broken, it prints the configured action.
...
Edit rules.json to define your monitoring rules. Each rule is an object with the following keys:
...
Before running the script, you must set the API credentials.
...
View Output:
The script will log in, list all VMs, and then print any alerts that are triggered.

--- Proxmox Monitor Script ---
Target URL: [https://proxmox.example.com:8006](https://proxmox.example.com:8006)
Target User: root@pam
Attempting login to [https://proxmox.example.com:8006](https://proxmox.example.com:8006) as root@pam...
Login SUCCESSFUL.

Loading monitoring rules from rules.json...
Successfully loaded 3 rules.
